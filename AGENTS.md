...
Runs on `http://localhost:3000`. NuxtHub auto-provisions an embedded SQLite database (libsql driver), blob storage, KV, and cache in dev mode — no external services required. Migrations under `.data/db/migrations/sqlite/` are applied automatically on startup.
...
- **Bun runtime location**: Bun is installed at `$HOME/.bun/bin/bun` and is on `PATH` in interactive shells (added to `~/.bashrc`). In non-interactive shells (e.g. some scripts), call it via the full path `$HOME/.bun/bin/bun` if `bun` is not found.
- **Local dev `.env`**: Create a `.env` (gitignored) with `NUXT_DEV_AUTH=true` to enable the no-OAuth dev login personas (see below). `NUXT_SESSION_PASSWORD` is auto-generated by `nuxt-auth-utils` in dev if unset.
